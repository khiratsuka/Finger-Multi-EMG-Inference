        ...
        for cname in self.class_name:
            for data_num in range(data_num_per_label[cname]):
                ...
                if len(one_label_path) != 0:
                    ...
                    correct_class.append(LABEL_NAMES_DICT[self.training_target][cname])
...
def createDataset(dataset_path, training_target, preprocess, isOnlyTest=False):
    ...
    train_size = 300*len(LABEL_ID[training_target]) if not training_target == 'finger' else int(0.9*len(trainval_dataset))
...
def createDataLoader(train_dataset, val_dataset, test_dataset, batch_size=1, isOnlyTest=False):
