...
def EMGLSTMQuantizationArgparse():
    parser = argparse.ArgumentParser(description='LSTM量子化モデル推論プログラム')
...
def main():
    args = EMGLSTMQuantizationArgparse()
    ...
    # モデルの宣言と読み込み
    net = quantize_model.EMG_Inference_Model_LSTM(input_size=CH_NUM, hidden_size=int(RAW_DATA_LENGTH/4), class_size=len(LABEL_ID[args.training_target]))
    ...
    # キャリブレーションと量子化
    net = torch.quantization.quantize_dynamic(net, {torch.nn.LSTM, torch.nn.Linear}, dtype=torch.qint8)
